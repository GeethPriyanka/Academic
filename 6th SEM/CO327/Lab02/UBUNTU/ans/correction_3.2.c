...
CO327 		Lab02				Exercise 3.2 part(b)(iii)
*/
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
	...
	if (pid > 0)		/// change is done here.. making this to a parent process.
	{
		...
		// execute grep
		if(execvp("grep", grep_args) == -1)		////////////never return from here
			die("execvp()");
	...
	else
	{
		...
		// execute cat
		if(execvp("cat", cat_args) == -1)	///////////never return from here
			die("execvp()");
